...
**Tier 3 Core** of the KPoI ladder, status: **integrating into the Coaching OS**. The PHP service at `coaching.tricktionary.com` is being integrated (backend + frontend) into the new standalone **Coaching OS** (forge.tricktionary.com/coaching; plan `~/.claude/plans/ok-so-big-ticklish-plum.md`) via a progressive strangler-fig migration — PHP stays fully live until each piece is re-homed, then retires to a marketing landing + funnel routes only. DO NOT propose new PHP work beyond that transition. **Correction (2026-07-21):** the older framing that this is 'absorbed as a Coaching feature inside the wingfoil Tricktionary mobile app (`tricktionary-apps/`)' was a merge idea Michi considered, not a decision — the actual home is the Coaching OS. Funnel pages stay (events, Q&A, private-inquiry, **and the `/api/waitlist.php` endpoint that the-lineup hub forms post to**).
